...
export function useState(initialValue) {
  ...
  const setState = (newValue) => {
    if (typeof newValue === 'function') {
      hooks[currentHookIndex] = newValue(hooks[currentHookIndex]);
    } else {
      hooks[currentHookIndex] = newValue;
    }
    
    // Trigger re-render
    if (currentComponent && currentComponent.scheduleRender) {
      currentComponent.scheduleRender();
    }
  };
...
export function mount(componentFn, container) {
  ...
  // Set up current component context for hooks
  currentComponent = { scheduleRender };
